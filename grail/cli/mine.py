...
async def generate_rollouts_for_window(
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Overlaps vLLM generation for problem N+1 with proof computation for N.
    # This saves ~2.79s per problem (vLLM time) by running both in parallel.
    #
    # Pipeline structure:
    # - pending_vllm_data: Tokens generated by vLLM, waiting for proof computation
    # - pending_metadata: Problem metadata (seed, nonce, block) for pending data
    # - Current iteration: compute proofs for pending, start vLLM for next

    pending_vllm_data: list[tuple[list[int], int, float, dict]] | None = None
    ...
    async def _process_pending_proofs() -> None:
        """Compute proofs for pending vLLM data and package rollouts."""
        nonlocal pending_vllm_data, pending_metadata
        nonlocal successful_rollouts, failed_rollouts, total_reward
        nonlocal text_logs_emitted

        if pending_vllm_data is None or pending_metadata is None:
            return

        pm = pending_metadata
        gen_start = pm["gen_start"]

        # Compute proofs in thread pool (the slow ~7s part)
        grpo_rollouts = await asyncio.to_thread(
            loop.compute_grpo_proofs_sync,
            pending_vllm_data,
            combined_randomness,
            wallet,
        )

        rollout_gen_duration = time.time() - pm["vllm_start"]
        ...
        logger.info(
            "GRPO batch: %s/%s successful, mean reward: %.3f, generation time: %.2fs (pipelined)",
            successful_count,
            len(grpo_rollouts),
            mean_reward,
    ...
    while True:
        ...
        # Atomically claim next problem from shared queue
        ...
        if is_leader:
            ...
            if current_block is not None:
                logger.debug("Worker %d using cached block %d", worker_config.worker_id, current_block)
            else:
                current_block = await subtensor.get_current_block()
        ...
        try:
            ...
            # ─────────────────────────────────────────────────────────────────
            # PIPELINED EXECUTION:
            # 1. Start vLLM generation for current problem (async)
            # 2. While vLLM runs, compute proofs for previous problem
            # 3. This overlaps ~2.79s vLLM time with ~7s proof time
            # ─────────────────────────────────────────────────────────────────

            vllm_start = time.time()
            # Start vLLM generation as async task
            vllm_task = asyncio.create_task(
                loop.prepare_grpo_generation_async(
                    _env_factory,
                    ROLLOUTS_PER_PROBLEM,
            ...
            # While vLLM generates, compute proofs for pending problem (if any)
            if pending_vllm_data is not None:
                logger.debug("🔄 Pipelining: computing proofs while vLLM generates next batch")
                await _process_pending_proofs()

            # Wait for vLLM to complete
            vllm_data = await vllm_task

            # Store for next iteration's proof computation
            pending_vllm_data = vllm_data
            pending_metadata = {
                "gen_start": gen_start,
    ...
    # Process final pending problem (loop exited before processing it)
    if pending_vllm_data is not None:
        logger.info("🔄 Processing final pending problem...")
        await _process_pending_proofs()
